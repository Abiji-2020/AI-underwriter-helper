...
In the insurance industry, trust and compliance are paramount. Traditional "Black Box" AI models that output a price without explanation are often unusable for regulatory reasons. Underwriters need to know _exactly_ how a premium was calculated.
...
2.  **The Assistant (The "Why")**: We use LLMs (OpenAI) _only_ to analyze the math and explain it in plain English.
...
4.  Run the development server:
    ...
    npm run dev
    ```
    _Note: To test the AI features locally, use `vercel dev` if you have the Vercel CLI installed, as Vite does not natively serve the `api/` directory._
